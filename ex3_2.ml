...
module BetterHeap (Element : ORDERED) : HEAP with type elem = Element.t =
  struct
    include Heap(Element)
...
module BetterIntHeap = BetterHeap(Int)
		     
let () =
  let open BatPrintf in
  let open IntHeap in
  assert(empty |> is_empty);
  assert(not (of_list [1] |> is_empty));
  let heap = of_list [1; 2; 4] in
  assert(heap = Tree(2, 1,
		     Tree(1, 2, Empty, Empty),
		     Tree(1, 4, Empty, Empty)));
  let heap = insert 3 heap in
  assert(heap = Tree(2, 1,
		     Tree(1, 2, Empty, Empty),
		     Tree(1, 3, Tree(1, 4, Empty, Empty), Empty)));
  let open BetterIntHeap in
  let heap = of_list [1; 2; 4] in
  assert(heap = Tree(2, 1,
		     Tree(1, 2, Empty, Empty),
		     Tree(1, 4, Empty, Empty)));
